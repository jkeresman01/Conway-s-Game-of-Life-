...
void Background::create(sf::RenderWindow *window, const std::filesystem::path &path)
{
    initBackground(window, path);
}

void Background::initBackground(sf::RenderWindow *window, const std::filesystem::path &path)
{
    setImage(path);
    setWindow(window);
}
